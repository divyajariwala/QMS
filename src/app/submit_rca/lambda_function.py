        ...
        with psycopg.connect(conn_string) as conn:
            with conn.cursor() as cur:
                ...
                for idx, rca in enumerate(rca_list):
                    ...
                    # Status flags (default to False if not provided)
                    is_ai_generated = rca.get('is_ai_generated', False)
                    is_added = rca.get('isAdded', False)
                    is_edited = rca.get('isEdited', False)
...
def lambda_handler(event, context):
    ...
    - isAdded: Boolean flag (true if this RCA was newly created/added by user, false otherwise)
    - isEdited: Boolean flag (true if this RCA was edited/modified by user, false otherwise)
    """
    try:
